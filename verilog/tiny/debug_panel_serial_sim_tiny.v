...
(
    ...
    reg btn_halt_d, btn_run_d, btn_step_d, btn_write_d, btn_read_d, btn_ping_d;
    ...
    wire control_write_event = p_dbg_toggle | p_static_toggle | p_halt | p_run | p_step;
    ...
    reg [23:0] cmd_shift;
    reg [4:0]  cmd_bits_left;
    reg [15:0] resp_shift;
    reg [4:0]  resp_bits_left;
    reg [23:0] pending_word;
    reg        pending_valid;
    reg [15:0] clkdiv_cnt;
    reg [3:0]  pending_cmd;
        ...
        if (!rst_n) begin
            ...
            if (!pending_valid && !busy) begin
                if (control_write_event) begin
                    pending_word  <= {CMD_WRITE, REG_CONTROL, control_word};
                    pending_cmd   <= CMD_WRITE;
                    pending_valid <= 1'b1;
                end else if (p_write) begin
                    pending_word  <= {CMD_WRITE, sw_addr, sw_wdata};
                    pending_cmd   <= CMD_WRITE;
                    pending_valid <= 1'b1;
                end else if (p_read) begin
                    pending_word  <= {CMD_READ, sw_addr, 16'h0000};
                    pending_cmd   <= CMD_READ;
                    pending_valid <= 1'b1;
                end else if (p_ping) begin
                    pending_word  <= {CMD_PING, 4'h0, 16'h0000};
            ...
            if (tick) begin
                case (state)
                    ...
                    S_CMD_FALL: begin
                        dbg_clk <= 1'b0;
                        if (cmd_bits_left == 5'd1) begin
                            dbg_data_oe <= 1'b0;
                            dbg_data_out <= 1'b1;
                            state <= S_TURNAROUND;
                        end else begin
                            cmd_shift <= {cmd_shift[22:0], 1'b0};
                            cmd_bits_left <= cmd_bits_left - 5'd1;
                            dbg_data_out <= cmd_shift[22];
                            state <= S_CMD_RISE;
                        end
                    end
