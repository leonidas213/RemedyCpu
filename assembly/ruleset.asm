...
instMemoryEnable=2
enableInterupt=3
GPI =4   ;works with interrupt
GPO =5
;random number generator config register
RandomSeedAddr =8        ;seed location
RandomAlwaysActiveAddr= 9    ;always active register
                            ;if 1 then RNG is always active at every clock cycle
                            ;if 0 then RNG is active only when Random nummber is requested
...
#ruledef 
{
    ...
    rdflg {rd:registers} => 0x46 @rd`4 @0`4
    
    wrflg {rs:registers} => 0x47 @0`4 @rs`4

    Rand {rd:registers} => 0x48 @rd`4 @0`4
    outfromConst {rd:registers} => 0x49 @rd`4 @0`4
    ...
    jmi2cNotBusy {value: i8} => 
    {   relad=(value-pc-1)
        0x4D @relad`8
    }
    jmi2cNotAck {value: i8} => 
    {   relad=(value-pc-1)
        0x4E @relad`8
    }
    jmi2cNotDone {value: i8} => 
    {   relad=(value-pc-1)
        0x4F @relad`8
    }
    ;--------------macros-----------------------
    ...
    ActivateAlwaysRandom => asm{
        ldi r12 ,1
        out RandomAlwaysActiveAddr , r12
    }
    DeactivateAlwaysRandom => asm{
        ldi r12 ,0
        out RandomAlwaysActiveAddr , r12
    }
    EnableI2C => asm{
    ...
    I2CWriteAddress{value : i8} =>asm{
         ldi r12 ,{value}
        out I2CwriteAddr , r12
    }
    I2CWriteBuffer{value : i8}=> asm{
         ldi r12 ,{value}
        out I2CwriteBuffer , r12
    }
    I2CWriteLen{value : i8} =>asm{
         ldi r12 ,{value}
        out I2CwriteLen , r12
    }
    I2CReadLen{value : i8}=> asm{
        ldi r12 ,{value}
        out I2CreadLen , r12
    }
    I2CReadWord {rd:registers} => asm{
